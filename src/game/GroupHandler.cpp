...
void WorldSession::HandleLootMethodOpcode(WorldPacket& recv_data)
{
	...
	if ((LootMethod)lootMethod == MASTER_LOOT || !group->GetLooterGuid())
...
void WorldSession::HandleGroupChangeSubGroupOpcode(WorldPacket& recv_data)
{
	...
	if (!group->HasFreeLotSubGroupReal(groupNr))
		return;
    /********************/
...
void WorldSession::HandleGroupSwapSubGroupOpcode(WorldPacket& recv_data)
{
	...
	Player* player1 = sObjectMgr.GetPlayer(unk1.c_str());
	Player* player2 = sObjectMgr.GetPlayer(unk2.c_str());
	ObjectGuid guid1 = sObjectMgr.GetPlayerGuidByName(unk1.c_str());
	ObjectGuid guid2 = sObjectMgr.GetPlayerGuidByName(unk2.c_str());
	if (player1)
		groupNr1 = player1->GetSubGroup();
	else
		groupNr1 = group->GetMemberGroup(guid1);

	if (player2)
		groupNr2 = player2->GetSubGroup();
	else
		groupNr2 = group->GetMemberGroup(guid2);
	...
	if (player1)
		group->ChangeMembersGroup(player1, groupNr2);
	else
		group->ChangeMembersGroup(guid1, groupNr2);

	if (player2)
		group->ChangeMembersGroup(player2, groupNr1);
	else
		group->ChangeMembersGroup(guid2, groupNr1);
}
