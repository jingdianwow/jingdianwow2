    ...
    */
    ...
    class NearestGameObjectEntryInObjectRangeCheck
    {
        public:
            ...
            bool operator()(GameObject* go)
            {
				if (go->GetEntry() == i_entry && i_obj.IsWithinDistInMap(go, i_range) && go->isSpawned())
                {
	...
	class AnyUnitInObjectCorpseRangeCheck
    {
        public:
			AnyUnitInObjectCorpseRangeCheck(WorldObject const* obj, float range) : i_obj(obj), i_range(range) {}
            WorldObject const& GetFocusObject() const { return *i_obj; }
            bool operator()(Unit* u)
            {
