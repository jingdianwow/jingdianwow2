...
class UnitDistance
{
    public:
		...
		bool operator < (const UnitDistance& rhs)
        {
			return DistancePoints < rhs.DistancePoints;
        }
};

typedef std::list<UnitDistance> UnitDistanceStandingList;
...
class Spell
{
    ...
    public:
		...
		int32 CalculateKongzhiTimeByResist(Unit* pCaster, Unit* pVictim, uint32 duration);
		void SetActualTime(uint32 time) { m_actual_time = time; }
		uint32 GetActualTime() { return m_actual_time; }
    ...
    protected:
        ...
        uint32 m_spellState;
        uint32 m_timer;
		uint32 m_KongZhitimer;
		uint32 m_actual_time;
            ...
            for (typename GridRefManager<T>::iterator itr = m.begin(); itr != m.end(); ++itr)
            {
                ...
                switch (i_push_type)
                {
                    ...
                    case PUSH_IN_FRONT_90:
                        ...
                        break;
					/*case PUSH_IN_FRONT_30:
						if (i_castingObject->isInFront((Unit*)(itr->getSource()), i_radius, M_PI_F / 5))
							i_data->push_back(itr->getSource());
						break;*/
                    case PUSH_IN_FRONT_15:
