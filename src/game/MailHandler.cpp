...
void WorldSession::HandleSendMail(WorldPacket& recv_data)
{
	...
	if (sWorld.getConfig(CONFIG_BOOL_SAFE_LOCK) && pl->GetPlayerSafe() == 0)
	{
		...
		pl->SendMailResult(0, MAIL_RETURNED_TO_SENDER, MAIL_ERR_INTERNAL_ERROR);
		return;
	}
    ...
    if (itemGuid || money > 0)
    {
        ...
        if (item)
        {
			...
			sLog.outTrade("sender = %s reciever = %s type = %s item_guid = %u item_entry = %u amount = %u method = %s", pl->GetName(), receiver.c_str(), "items", item->GetGUIDLow(), item->GetEntry(), item->GetCount(), "mail");
        }

		if (money > 0)
			sLog.outTrade("sender = %s reciever = %s type = %s item_guid = %u item_entry = %u amount = %u method = %s", pl->GetName(), receiver.c_str(), "gold", 0, 0, money, "mail");
