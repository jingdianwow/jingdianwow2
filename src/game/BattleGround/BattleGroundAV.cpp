...
void BattleGroundAV::HandleKillUnit(Creature* creature, Player* killer)
{
    ...
    switch (event1)
    {
        ...
        case BG_AV_BOSS_H:
            ...
            RewardHonorToTeam(168, ALLIANCE);
        ...
        case BG_AV_CAPTAIN_H:
            ...
            RewardHonorToTeam(126, ALLIANCE);
...
void BattleGroundAV::EndBattleGround(Team winner)
{
    ...
    for (uint8 i = 0; i < PVP_TEAM_COUNT; ++i)
    {
        if (tower_survived[i])
        {
            ...
            RewardHonorToTeam((tower_survived[i] * 84), team[i]);
        }
        DEBUG_LOG("BattleGroundAV: EndbattleGround: bgteam: %u towers:%u honor:%u rep:%u", i, tower_survived[i], GetBonusHonorFromKill(tower_survived[i] * 42), tower_survived[i] * BG_AV_REP_SURVIVING_TOWER);
        if (graves_owned[i])
...
void BattleGroundAV::EventPlayerDestroyedPoint(BG_AV_Nodes node)
{
    ...
    if (IsTower(node))
    {
        ...
        RewardHonorToTeam(126, ownerTeam);
...
void BattleGroundAV::Reset()
{
    ...
    m_HonorMapComplete    = (isBGWeekend) ? BG_AV_KILL_MAP_COMPLETE_HOLIDAY : BG_AV_KILL_MAP_COMPLETE_HOLIDAY;
    m_RepTowerDestruction = (isBGWeekend) ? BG_AV_REP_TOWER_HOLIDAY         : BG_AV_REP_TOWER_HOLIDAY;
    m_RepCaptain          = (isBGWeekend) ? BG_AV_REP_CAPTAIN_HOLIDAY       : BG_AV_REP_CAPTAIN_HOLIDAY;
    m_RepBoss             = (isBGWeekend) ? BG_AV_REP_BOSS_HOLIDAY          : BG_AV_REP_BOSS_HOLIDAY;
    m_RepOwnedGrave       = (isBGWeekend) ? BG_AV_REP_OWNED_GRAVE_HOLIDAY   : BG_AV_REP_OWNED_GRAVE_HOLIDAY;
    m_RepSurviveCaptain   = (isBGWeekend) ? BG_AV_REP_SURVIVING_CAPTAIN_HOLIDAY : BG_AV_REP_SURVIVING_CAPTAIN_HOLIDAY;
    m_RepSurviveTower     = (isBGWeekend) ? BG_AV_REP_SURVIVING_TOWER_HOLIDAY : BG_AV_REP_SURVIVING_TOWER_HOLIDAY;
    m_RepOwnedMine        = (isBGWeekend) ? BG_AV_REP_OWNED_MINE_HOLIDAY    : BG_AV_REP_OWNED_MINE_HOLIDAY;
