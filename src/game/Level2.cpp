...
/*{
    if (m_session)
    {
        ...
        if (!subpart)
        {
            ...
            dist_buf[0] = '\0';
		sLog.outError("ShowTriggerTargetListHelper: id = %u, mapId = %u, target_X = %f, target_Y = %f, target_Z = %f, dist_buf = %s", id, at->target_mapId, at->target_X, at->target_Y, at->target_Z, dist_buf);
	...
	else
	{
		...
		sLog.outError("ShowTriggerTargetListHelper: id = %u, mapId = %u, target_X = %f, target_Y = %f, target_Z = %f", id, at->target_mapId, at->target_X, at->target_Y, at->target_Z);
	}
}*/
