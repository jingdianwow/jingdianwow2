...
void WorldSession::HandleGuildDisbandOpcode(WorldPacket& /*recvPacket*/)
{
    ...
    guild->Disband();
    delete guild;
	GetPlayer()->m_Player_GuiId[PLAYED_GUIID_MIANFEI] = 0;
	GetPlayer()->m_Player_GuiId[PLAYED_GUIID_SHOUFEI] = 0;
	GetPlayer()->m_Player_GuiId[PLAYED_GUIID_MIANFEITIME] = 0;
	GetPlayer()->SaveToDB();
...
void WorldSession::HandleGuildLeaderOpcode(WorldPacket& recvPacket)
{
	...
	if (player)
	{
		player->m_Player_GuiId[PLAYED_GUIID_MIANFEI] = oldLeader->GetPlayerGuiIdMianFei();
		player->m_Player_GuiId[PLAYED_GUIID_SHOUFEI] = oldLeader->GetPlayerGuiIdShouFei();
		player->m_Player_GuiId[PLAYED_GUIID_MIANFEITIME] = oldLeader->GetPlayerGuiIdMianFeiTime();
		oldLeader->m_Player_GuiId[PLAYED_GUIID_MIANFEI] = 0;
		oldLeader->m_Player_GuiId[PLAYED_GUIID_SHOUFEI] = 0;
		oldLeader->m_Player_GuiId[PLAYED_GUIID_MIANFEITIME] = 0;
		player->SaveToDB();
		oldLeader->SaveToDB();
	}
	else
	{
		LoginDatabase.PExecute("UPDATE characters_vip SET mianfei = %u, shoufei = %u, mianfeitime = %u WHERE guid = %u", oldLeader->GetPlayerGuiIdMianFei(), oldLeader->GetPlayerGuiIdShouFei(), oldLeader->GetPlayerGuiIdMianFeiTime(), slot->guid.GetCounter());
