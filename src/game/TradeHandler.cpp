    ...
    for (int i = 0; i < TRADE_SLOT_TRADED_COUNT; ++i)
    {
        ...
        if (traderCanTrade && playerCanTrade)
        {
            ...
            if (myItems[i])
            {
				...
				sLog.outTrade("sender = %s reciever = %s type = %s item_guid = %u item_entry = %u amount = %u method = %s", _player->GetName(), trader->GetName(), "Items", myItems[i]->GetGUIDLow(), myItems[i]->GetEntry(), myItems[i]->GetCount(), "transport");
            }
            ...
            if (hisItems[i])
            {
				...
				sLog.outTrade("sender = %s reciever = %s type = %s item_guid = %u item_entry = %u amount = %u method = %s", trader->GetName(), _player->GetName(), "Items", hisItems[i]->GetGUIDLow(), hisItems[i]->GetEntry(), hisItems[i]->GetCount(), "transport");
            }
        }
        else
        {
...
void WorldSession::HandleAcceptTradeOpcode(WorldPacket& recvPacket)
{
    ...
    if (his_trade->IsAccepted())
    {
		...
		if (my_trade->GetMoney() > 0)
			sLog.outTrade("sender = %s reciever = %s type = %s item_guid = %u item_entry = %u amount = %u method = %s", _player->GetName(), trader->GetName(), "gold", 0, 0, my_trade->GetMoney(), "transport");

		if (his_trade->GetMoney() > 0)
			sLog.outTrade("sender = %s reciever = %s type = %s item_guid = %u item_entry = %u amount = %u method = %s", trader->GetName(), _player->GetName(), "gold", 0, 0, his_trade->GetMoney(), "transport");
...
void WorldSession::HandleInitiateTradeOpcode(WorldPacket& recvPacket)
{
	...
	if (sWorld.getConfig(CONFIG_BOOL_SAFE_LOCK) && pOther->GetPlayerSafe() == 0)
	{
		ChatHandler(pOther).PSendSysMessage(LANG_SAFE_20);
		return;
	}
