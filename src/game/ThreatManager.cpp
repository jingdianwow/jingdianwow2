...
float ThreatCalcHelper::CalcThreat(Unit* pHatedUnit, Unit* /*pHatingUnit*/, float threat, bool crit, SpellSchoolMask schoolMask, SpellEntry const* pThreatSpell)
{
	...
	if (pThreatSpell && pThreatSpell->SpellFamilyFlags & UI64LIT(0x0000000040000001))
...
void ThreatManager::addThreat(Unit* pVictim, float pThreat, bool crit, SpellSchoolMask schoolMask, SpellEntry const* pThreatSpell)
{
	...
	{
		if (((Player*)pVictim)->ChouhenDebug())
		{
			sLog.outBug("(pVictim:Name = %s pVictim->GUIDLow = %u) (getOwner():Name = %s GUIDLow = %u %f %f %f %u) ChouhenDebug() %u", pVictim->GetName(), pVictim->GetGUIDLow(), getOwner()->GetName(), getOwner()->GetGUIDLow(), getOwner()->GetPositionX(), getOwner()->GetPositionY(), getOwner()->GetPositionZ(), getOwner()->GetMapId(), ((Player*)pVictim)->ChouhenDebug());
			ChatHandler(((Player*)pVictim)).PSendSysMessage(LANG_GROUP_3, pVictim->GetName(), pVictim->GetGUIDLow(), getOwner()->GetName(), getOwner()->GetGUIDLow(), getOwner()->GetPositionX(), getOwner()->GetPositionY(), getOwner()->GetPositionZ(), getOwner()->GetMapId());
		}
	}
