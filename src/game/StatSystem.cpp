...
void Pet::UpdateAttackPowerAndDamage(bool ranged)
{
    ...
    if (owner && owner->GetTypeId() == TYPEID_PLAYER)
    {
		...
		if (getPetType() == HUNTER_PET && owner->getClass() == CLASS_HUNTER)
		{
			bonusAP = owner->GetTotalAttackPowerValue(RANGED_ATTACK) * 0.15f;
			SetBonusDamage(int32(owner->GetTotalAttackPowerValue(RANGED_ATTACK) * 0.06f));
		}
		else
