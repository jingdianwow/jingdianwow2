...
void Group::MasterLoot(WorldObject* pSource, Loot* loot)
{
    ...
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
    {
		...
		if (pSource->GetTypeId() == TYPEID_UNIT) 
		{
			...
			if (pCreature && pCreature->CanLoot(looter))
			{
        ...
        if (!roll->playerVote.empty())
        {
            ...
            if (player && player->GetSession())
            {
                ...
                if (msg == EQUIP_ERR_OK)
                {
					...
					if (newitem && item)
					{
						switch (roll->lootedTargetGUID.GetHigh())
						{
						case HIGHGUID_GAMEOBJECT:
							if (GameObject* go = player->GetMap()->GetGameObject(roll->lootedTargetGUID))
							{
								if (go)
								{
							...
							break;
						case HIGHGUID_UNIT:
							if (Creature* pCreature = player->GetMap()->GetCreature(roll->lootedTargetGUID))
							{
...
void Group::UpdatePlayerOutOfRange(Player* pPlayer)
{
    ...
    for (GroupReference* itr = GetFirstMember(); itr != NULL; itr = itr->next())
        if (Player* player = itr->getSource())
			if (player != pPlayer && !player->IsInVisibleList(pPlayer))
                player->GetSession()->SendPacket(&data);
}
...
void Group::ResetInstances(InstanceResetMethod method, Player* SendMsgTo)
{
    ...
    for (BoundInstancesMap::iterator itr = m_boundInstances.begin(); itr != m_boundInstances.end();)
    {
        ...
        if (isEmpty || method == INSTANCE_RESET_GROUP_DISBAND)
        {
			...
			if (state->CanReset())
			{
				if (state->GetMapId() == 309 || state->GetMapId() == 409 || state->GetMapId() == 469 || state->GetMapId() == 249)
					sLog.outBug("method = %u", method);
			...
			if (sWorld.getConfig(CONFIG_BOOL_PLAYER_INSTANCES_PER_HOUR_DAKAI) && entry->IsNonRaidDungeon())
			{
				QueryResult* result = LoginDatabase.PQuery("SELECT memberGuid FROM group_member WHERE groupId = %u", GetId());
				if (result)
				{
					do
					{
						...
						ObjectGuid guid = ObjectGuid(HIGHGUID_PLAYER, memberGuid);
						Player* player = sObjectMgr.GetPlayer(guid);
						if (player)
						{
							player->m_Player_Instances[PLAYED_INSTANCES_CONT] += 1;
							if (player->m_Player_Instances[PLAYED_INSTANCES_TIME] == 0)
								player->m_Player_Instances[PLAYED_INSTANCES_TIME] += sWorld.GetGameTime() + 3600;
							ChatHandler(player).PSendSysMessage(ZHANYOU_TIME_19, player->m_Player_Instances[PLAYED_INSTANCES_CONT] > 5 ? 5 : player->m_Player_Instances[PLAYED_INSTANCES_CONT], sWorld.getConfig(CONFIG_UINT32_PLAYER_INSTANCES_PER_HOUR), player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 1), player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 2), player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 3), player->GetDaysTime(player->m_Player_Instances[PLAYED_INSTANCES_TIME], 4));
						}
						else
						{
							QueryResult* result = LoginDatabase.PQuery("SELECT InstancesTime FROM characters_vip WHERE guid = %u", memberGuid);
							if (result)
							{
								Field *fields = result->Fetch();
								uint32 InstancesTime = fields[0].GetUInt32();
								if (InstancesTime == 0)
									CharacterDatabase.PExecute("UPDATE characters_vip SET InstancesCont = InstancesCont + '%u', InstancesTime = '%u' WHERE guid = '%u'", 1, sWorld.GetGameTime() + 3600, memberGuid);
								else
									CharacterDatabase.PExecute("UPDATE characters_vip SET InstancesCont = InstancesCont + '%u' WHERE guid = '%u'", 1, memberGuid);
							}
						}
...
static void RewardGroupAtKill_helper(Player* pGroupGuy, Unit* pVictim, uint32 count, bool PvP, float group_rate, uint32 sum_level, bool is_dungeon, Player* not_gray_member_with_max_level, Player* member_with_max_level, uint32 xp)
{
    ...
    if (!PvP)
    {
		...
		/*sGet.SetGroupItem(pGroupGuy, pVictim, count);
		if (pGroupGuy->GetGuildId() && pGroupGuy->getLevel() == 60 && count >= 2 && pGroupGuy->GetPlayerGuild(pGroupGuy->GetGuildId(), 2) < 2000000000 && pGroupGuy->GetPlayerGuildCaiLiao(pGroupGuy->GetPlayerGuild(pGroupGuy->GetGuildId(), 1), 1) >= pGroupGuy->GetPlayerGuild(pGroupGuy->GetGuildId(), 2) && pGroupGuy->GetPlayerGuild(pGroupGuy->GetGuildId(), 1) < 5)
	    {
		    if (pVictim->GetTypeId() == TYPEID_UNIT)
		    {
			    if (((Creature*)pVictim)->GetCreatureInfo()->Rank == 1)
			    {
				    if (xp == 0)
					    return;
			        ...
			        pGroupGuy->SetGuildXp(pGroupGuy, pGroupGuy->GetGuildXp(pGroupGuy->GetGuildId()) + guildxp);
			        ChatHandler(pGroupGuy).PSendSysMessage(LANG_GONGGAO_4, guildxp);
			    }
				else
				if (((Creature*)pVictim)->GetCreatureInfo()->Rank == 3)
			    {
				    if (xp == 0)
					    return;
...
bool Group::HasFreeLotSubGroupReal(uint8 groupNr)
{
	uint32 count = 0;
	for (member_citerator itr = GetMemberSlots().begin(); itr != GetMemberSlots().end(); ++itr)
	{
		if (!itr->guid)
			continue;
