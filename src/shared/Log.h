...
class Log : public MaNGOS::Singleton<Log, MaNGOS::ClassLevelLockable<Log, ACE_Thread_Mutex> >
{
        ...
        ~Log()
        {
			...
			if (BattleGroundLog != NULL)
				fclose(BattleGroundLog);
			BattleGroundLog = NULL;
        }
    public:
        void Initialize();
		...
		void outTrade(const char* str, ...)       ATTR_PRINTF(2, 3);

		void outLoot(const char* str, ...)        ATTR_PRINTF(2, 3);

		void outBug(const char* str, ...)        ATTR_PRINTF(2, 3);

		void outBattleGround(const char* str, ...)        ATTR_PRINTF(2, 3);
