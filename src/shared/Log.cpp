...
Log::Log() :
raLogfile(NULL), logfile(NULL), gmLogfile(NULL), charLogfile(NULL), TradeLog(NULL), LootLog(NULL), BugLog(NULL), BattleGroundLog(NULL),
...
void Log::Initialize()
{
	...
	TradeLog = openLogFile("TradeLog", "TradeLogTimestamp", "a");
	LootLog = openLogFile("LootLog", "LootLogTimestamp", "a");
	BugLog = openLogFile("BugLog", "BugLogTimestamp", "a");
	BattleGroundLog = openLogFile("BattleGroundLog", "BattleGroundLogTimestamp", "a");
...
void Log::outTrade(const char* str, ...)
{
	if (!str)
        return;
    ...
    {
        va_list ap;
		outTimestamp(TradeLog);
        va_start(ap, str);
		vfprintf(TradeLog, str, ap);
		fprintf(TradeLog, "\n");
        va_end(ap);
		fflush(TradeLog);
    }
...
void Log::outLoot(const char* str, ...)
{
	...
	{
		va_list ap;
		outTimestamp(LootLog);
		va_start(ap, str);
		vfprintf(LootLog, str, ap);
		fprintf(LootLog, "\n");
		va_end(ap);
		fflush(LootLog);
	}
...
void Log::outBug(const char* str, ...)
{
	if (!str)
		return;
...
void Log::outBattleGround(const char* str, ...)
{
	if (!str)
		return;
	...
	{
		va_list ap;
		outTimestamp(BattleGroundLog);
		va_start(ap, str);
		vfprintf(BattleGroundLog, str, ap);
		fprintf(BattleGroundLog, "\n");
		va_end(ap);
		fflush(BattleGroundLog);
	}
}
