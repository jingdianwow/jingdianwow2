...
std::string Master::GetIplist(uint32 guid)
{
...
uint32 Master::GetIpPort(uint32 guid)
{
	QueryResult *result = IpVerifyDatabase.PQuery("SELECT Port FROM realmlist where id = '%u'", guid);
	if (result)
	{
		Field *fields = result->Fetch();
		uint32 Port = fields[0].GetUInt32();
		delete result;
		return Port;
	}
	else
